package com.example.springbootblog.common.persistence;
...
import com.example.springbootblog.enums.PropertyTypeEnum;
import com.example.springbootblog.util.ServletUtils;
...
public class PropertyFilter {
	...
	public PropertyFilter(final String filterName, final String value) {
		...
		try {
			matchType = Enum.valueOf(MatchTypeEnum.class, matchTypeCode.toUpperCase());
		} catch (RuntimeException e) {
			throw new IllegalArgumentException("filter名称" + filterName + "没有按规则编写,无法得到属性比较类型.", e);
		}

		try {
			propertyClass = Enum.valueOf(PropertyTypeEnum.class, propertyTypeCode.toUpperCase()).getValue();
		} catch (RuntimeException e) {
		...
		String modified = Pattern.compile(PropertyFilter.OR_SEPARATOR, Pattern.CASE_INSENSITIVE).matcher(propertyNameStr).replaceAll(PropertyFilter.OR_SEPARATOR);
		propertyNames = modified.split(PropertyFilter.OR_SEPARATOR);
